...
import { ChipSituacao, DataTexto, nomePessoa, Vazio } from "./comum";
...
export function Painel({ onAbrirAtividade }: { onAbrirAtividade: (id: string) => void }) {
  ...
  const comSituacao = dados.etapas.map((e) => ({ etapa: e, situacao: situacaoEtapa(e, dados.etapas, dataHoje) }));
  ...
  const concluidasMes = dados.etapas.filter((e) => e.status === "Concluída" && e.dataConclusao?.startsWith(mesAtual));

  const comSla = dados.etapas.filter((e) => e.status === "Concluída" && e.dataConclusao && prazoSla(e));
  ...
  const atividadesAbertas = dados.atividades.filter((a) => a.status !== "Concluída").length;
